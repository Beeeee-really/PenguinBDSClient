...
/** 查在线格式化纯文本（Markdown 关闭/不可用时的输出）。 */
function formatOnlineText(players) {
    if (!players.length) return '当前无玩家在线';
    return '当前在线：' + players.length + ' 人\n' + players.join('、');
}
...
const COMMANDS = [
    ...
    },
    {
        ...
        execute(ctx) {
            ...
            // 自定义 Markdown（msg_type=2，官方已向所有机器人开放）：解析玩家列表 → Markdown 卡片；
