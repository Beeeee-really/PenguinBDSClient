...
/** 渲染查在线的 Markdown 消息内容（msg_type=2）。 */
function buildOnlineMarkdown(bot, players) {
    const serverName = bot.config.getString('serverName', '') ||
        bot.config.getString('bot.name', 'HuHoBot');
    const lines = [];
    lines.push('# ' + serverName + ' 在线玩家');
    lines.push('');
...
const COMMANDS = [
    ...
    },
    {
        name: '查白名单',
        execute(ctx) {
            runGameCommand(ctx.bot, ctx, 'whitelist list');
        }
    },
    {
        name: '绑定白名单',
